...
export class ShelbyService {
  ...
  private publicUrl(blobName: string): string {
    // Full HTTP URL for the image — used in metadata JSON so image renders
    return `${SHELBYNET_RPC_BASE}/shelby/v1/blobs/${this._accountAddress}/${blobName}`;
  }

  private shelbyUri(blobName: string): string {
    // Store as account/blobName path — the contract prepends the gateway URL
    // This is also used as the CID passed to mint() on the smart contract
    return `${this._accountAddress}/${blobName}`;
  }
  ...
  static shelbyUriToHttpUrl(uri: string): string {
    if (!uri) return uri;
    // Strip shelby:// prefix if present (old format)
    const path = uri.startsWith("shelby://") ? uri.slice(9) : uri;
    return `${SHELBYNET_RPC_BASE}/shelby/v1/blobs/${path}`;
  }
